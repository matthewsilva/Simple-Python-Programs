...
def scheduleProcs(tslice, cpuQ):
    ...
    while (cpuQ.empty() != True):
        ...
        # Separate the process ID, execution time, and priority from the process info
        PID, exectime, priority = proc.split(",")
        
        # Convert exectime to an integer
        exectime = int(exectime)

        # Getting the proper time slice according to process priority
        if priority == "H":
            prioritySlice = tslice*3
        elif priority == "M":
            prioritySlice = tslice*2
        else:
            prioritySlice = tslice
        
        print("Getting next process - Process ", PID," of priority ", priority," has ", exectime," instructions to execute")
        
        # Initialize the timer
        timer = 0
        
        # While proc still has time in slice and still has code to execute
        while (timer < prioritySlice) and (exectime > 0):
        ...
        if (exectime > 0):
            ...
            # Create string with new exec time, process ID, and priority
